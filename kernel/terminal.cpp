...
Rectangle<int> Terminal::InputKey(
    ...
    if (ascii == '\n') {
        ...
        linebuf_index_ = 0;
        cmd_history_index_ = -1; // Enter 入力直後は履歴の index をリセット
        cursor_.x = 0;
        if (cursor_.y < kRow - 1) {
            ++cursor_.y;
        } else {
...
void Terminal::ExecuteLine() {
    ...
    if (strcmp(command, "echo") == 0) {
        ...
        FillRectangle(*window_->InnerWriter(), {4, 4}, {8*kColumns, 16*kRow}, {0, 0, 0});
        cursor_.y = 0;
...
void Terminal::ExecuteFile(const fat::DirectoryEntry& file_entry) {
    ...
    while (cluster != 0 && cluster != fat::kEndOfClusterchain) {
        ...
        memcpy(p, fat::GetSectorByCluster<char>(cluster), copy_bytes);
