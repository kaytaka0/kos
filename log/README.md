...
## 2025/04/06
- day17a
  - ボリュームを読み出して、バイト列をそのまま画面に表示するプログラムの追記を行う節
  - ボリュームの読み出しには Block I/O Protocol を使う。そのため久しぶりにカーネルローダ (Main.c) のプログラムに追記を行った。
  - カーネルローダのビルドに edk2 を使っているのだが、これまで使いやすいビルド環境が整っていなかったので毎回試行錯誤してビルド失敗したりすることが多かった。この機会に Docker コンテナ内で edk2 ビルド環境を整えることができた..!
  - ただ、 docker compse up --build でのツールインストールに時間がかかるので、マルチステージビルドを使ってキャッシュ再利用をうまくできるようにしてビルド時間短縮もしたいな。
  
## 2025/04/13
- day18b
  - day15a で疑問に思っていた、sti してない問題がここで解決した
  - ternimal.cpp での task.ReceiveMessage() を実行している箇所にはなるが、ここでもこれまでのサンプルコードでは cli だけ実行して、 sti は実行されていなかった。
  - おそらく、sti を書いていなかったことに特段の理由はなさそう。第 18 章の文章を読むと、 sti がないことで割り込み禁止のままプログラムが進んでしまって不具合が出ていたようだった。この章で sti を追記することで、この不具合を解消していた
  - sti してないと、タスク切換えタイマのタイムアウト割り込みも発生しなくなってしまう模様
  - 逆に今までなんで動いてたんだろうか。。ちゃんと理解しておきたいところ


## 2026/03/22
- day18b
  - onlyhlt のアプリのビルドも必要になったので、全体のビルドスクリプトと起動スクリプトを大幅に変更する
  - build.sh run でイメージの作成、qemuの起動を実施するようにした。
- day18c
  - rpn アプリを新規作成。逆ポーランド記法で電卓として使えるプログラム
  - ちょっと加算のテストしてみたところ、加算する数が大きくなると結果が正しくない
  - どうやら main() の戻り値は 8 bit でしか扱われないらしい。255 より大きくなると値がオーバーフローして 0 に戻るっぽい。
  - ちなみに 戻り値は `echo $?`, fish の場合は `echo $status` で確認できる。
  - アプリ自体は想定動作ということで OK
  - このアプリ用の Makefile を作っておく
```
kaytaka@msform ~/s/g/k/k/a/rpn (main)> ./a.out 140 22 + ; echo $status                                                                                                                  
162
kaytaka@msform ~/s/g/k/k/a/rpn (main)> ./a.out 2 4 + ; echo $status                                                                                                                     
6
kaytaka@msform ~/s/g/k/k/a/rpn (main)> ./a.out 140 140 + ; echo $status                                                                                                                 
24
# ??? 計算結果が正しくない
```

- day18c
  - rpn アプリを自作 OS 内で実行するためには、コマンドライン引数をバイナリに引き渡す機能も必要
  - ということで terminal.cpp に修正を加えた
  - カーネルをビルドしようとしたら、ライブラリがないとコンパイルエラーが発生する。。。なんでや
  - あー、使ってるライブラリは libc じゃなくて独自の newlib か。毎回これ忘れてるな。AI エージェントに指摘してもらわないと沼にハマってたぞ、大丈夫か。。。
  - newlib のためのフラグ環境変数の読み込みは自動化しておいたほうがいいな。
  - 一旦後回しで動作確認
  - カーネルのビルドはとおった。が、rpn アプリを実行すると仮想マシンが終了する。
  - カーネルコードについては、mikanos 本家リポジトリと比較しても変わらない。Makefile の内容も同じ。
  - mikanos 本家のカーネルコードをビルド & KosLoader で動作確認してみる。→ rpn アプリを実行するとやはり仮想マシンが落ちる。ローダの問題かも？あとはビルド環境の問題？
  - たとえば clang++ とか ld.ldd のバージョン違いによって、生成されるバイナリに差異が出たりするのかな